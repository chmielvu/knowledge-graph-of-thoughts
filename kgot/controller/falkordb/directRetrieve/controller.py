...
#
# Main authors: Andrea Jiang, Lorenzo Paleari, FalkorDB adaptation
...
from kgot.controller.falkordb.directRetrieve.llm_invocation_handle import (
...
class Controller(ControllerInterface):
    """FalkorDB directRetrieve Controller - loads entire graph into LLM context."""
    ...
    def _iterative_next_step_logic(self, problem: str, *args, **kwargs) -> Tuple[str, int]:
        ...
        while current_iteration < self.max_iterations:
            ...
            if retrieve_next_step['RETRIEVE'] > retrieve_next_step['INSERT']:
                temp_solutions = []
                for retrieve_query in retrieve_next_step['RETRIEVE_CONTENT']:
                    if any(kw in retrieve_query[:20] for kw in ['MATCH', 'SELECT', 'COUNT', 'RETURN']):
                        temp_solutions.extend(self._perform_retrieve_branch(problem, existing_entities_and_relationships, retrieve_query))
                    else:
                        temp_solutions.append(retrieve_query)
                raw_solutions.extend(temp_solutions)
                break
    ...
    def _retrieve_logic(self, query, existing_entities_and_relationships, current_iteration, solutions):
        if current_iteration == self.max_iterations and len(solutions) == 0:
            ...
            solutions.extend(forced_solution)
        ...
        if solutions:
            ...
            return 'No solution found', current_iteration
        return solution
