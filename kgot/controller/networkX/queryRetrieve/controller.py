...
from kgot.controller.networkX.queryRetrieve.llm_invocation_handle import (
        ...
        for retrieve_query in retrieve_queries:
            ...
            while ((not get_result[1] or is_empty_solution(solution))
                ...
                while not get_result[1] and not (get_result[1] and is_empty_solution(
                    ...
                    fix_retry_i += 1
                    self.logger.info(
                        f"Failed the retrieve query. Trying to fix the code. Retry number: {fix_retry_i} out of {self.max_cypher_fixing_retry} of code query fixes")
                    self.logger.error(
                        f"Trying to fix error encountered when executing RETRIEVE code: {retrieve_query}\nError: {get_result[2]}")
                    retrieve_query = fix_code(self.llm_planning, retrieve_query, get_result[2], existing_entities_and_relationships,
                                                self.usage_statistics)
                    self.logger.info(f"Fixed RETRIEVE query: {retrieve_query}")
                    get_result = self.graph.get_query(retrieve_query)
                    self.logger.info(f"Retrieved result after fix: {get_result}")
