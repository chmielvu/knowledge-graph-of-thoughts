...
from kgot.controller.neo4j.directRetrieve.llm_invocation_handle import (
        ...
        while current_iteration < self.max_iterations:
            ...
            if retrieve_next_step["RETRIEVE"] > retrieve_next_step["INSERT"]:
                ...
                for retrieve_query in retrieve_next_step["RETRIEVE_CONTENT"]:
                    if any(keyword in retrieve_query[:20] for keyword in ["MATCH", "SELECT", "COUNT", "RETURN"]):
                        temp_solutions.extend(self._perform_retrieve_branch(problem, existing_entities_and_relationships,
                                                  retrieve_query))
                    else:
                        temp_solutions.append(retrieve_query)
                raw_solutions.extend(
                    temp_solutions)  # Add the retrieve queries to the list of solutions
                break
